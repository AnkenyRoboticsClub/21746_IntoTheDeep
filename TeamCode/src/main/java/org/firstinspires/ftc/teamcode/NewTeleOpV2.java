...
@TeleOp(name = "TestTeleOpV2", group = "TeleOp")
public class NewTeleOpV2 extends LinearOpMode {
    private List<Action> runningActions = new ArrayList<>();
    public LazyImu lazyImu;
    private Limelight3A limelight;
    boolean driverControlled = true;
    boolean automaticPoseUpdate = true;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        //Imu
        lazyImu = new LazyImu(hardwareMap, "imu", new RevHubOrientationOnRobot(MecanumDrive.PARAMS.logoFacingDirection, MecanumDrive.PARAMS.usbFacingDirection));
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            if (result != null && result.isValid()) {
                ...
                if (botpose != null) {
                    ...
                    //fix roadrunner pose
                    if (driver1.wasJustPressed(GamepadKeys.Button.X)||(automaticPoseUpdate&&driverControlled)) {
                    ...
                    //Adding back vibration for driving
                    //red
                    double xdiff1 = Math.abs(xin-(-49));
                    double ydiff1 = Math.abs(yin-(-49));
                    //blue
                    double xdiff2 = Math.abs(xin-(49));
                    double ydiff2 = Math.abs(yin-(49));
            ...
            driverControlled = !(driver1.getButton(GamepadKeys.Button.DPAD_LEFT)||driver1.getButton(GamepadKeys.Button.DPAD_UP)||driver1.getButton(GamepadKeys.Button.DPAD_DOWN));
            if (driver1.wasJustPressed(GamepadKeys.Button.DPAD_LEFT)){
                ...
                TrajectoryActionBuilder upperSub = drive.actionBuilder(drive.pose)
                        .strafeToLinearHeading(new Vector2d(-25, 7), Math.toRadians(offsetOrientation))
                        ;
                runningActions.add(new ParallelAction(
                        upperSub.build()
                ));
                ...
                TrajectoryActionBuilder lowerSub = drive.actionBuilder(drive.pose)
                        .strafeToLinearHeading(new Vector2d(-25, -11), Math.toRadians(offsetOrientation))
                        ;
                runningActions.add(new ParallelAction(
                        lowerSub.build()
                ));
